...
-------- 3. create new login >> security folder of the server
...
-------- 5. create schema to add all wanted tables within 
-------- 6. alter schema >> adding all objects into 
-------- 7. add user to the schema 
-------- 8. set permissions for user 
-------- 9. disconnect as admin and login as user 
-------- 10. create new query file to excute queries 
...
------------ permissions ------------
-- role >> public >> no access 
--      >> sysadmin >> all access
--      >> serveradmin >> all access but can be denied from the admin 

-- Grant VS with Grant 
-- grant >> to give the access to only one user 
-- with Grant >> to give the user access and he can give access to any user else 
